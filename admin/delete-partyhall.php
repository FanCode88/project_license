...
if (isset($_POST['Delete']) && isset($_POST['partyhall'])) {
    ...
    $stmt = mysqli_prepare($link, "DELETE FROM partyhalls WHERE partyhall_id = ?");
    mysqli_stmt_bind_param($stmt, "i", $partyhall_id);
    ...
    if ($result) {
        ...
        mysqli_stmt_close($stmt);
        mysqli_close($link);
        die("There was a problem while deleting the partyhall ... \n" . $error);
    }
} else {
    mysqli_close($link);
